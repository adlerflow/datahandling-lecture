...
renderPlot({
     ...
     # Find major counts
  df <- student_data %>% 
    mutate(literacy = as.numeric(literacy)-1) |> 
  ...
  student_data %>% 
    ...
    geom_point(stat = "identity", color = "#02934a", size = 2) +
    geom_point(aes(y = mean_literacy), color = "red", shape=1, size=5) +
    geom_point(aes(y = mean_literacy), color = "red", shape=1, size=1) +
    scale_y_continuous(limits = c(0, 10)) +
    geom_hline(aes(yintercept = mean(as.numeric(student_data$literacy))), 
               color = "gray70", linetype = 3) +
    theme_classic()
